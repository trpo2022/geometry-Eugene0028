...
void circle_f(char str[]){
	...
	printf("%s\n", str);
	if ((for_circle_area(digit_circle) && for_circle_perimetr(digit_circle)) > 0){printf("\nAREA CIRCLE: %.2f\nPERIMETR CIRCLE: %.2f\n", for_circle_area(digit_circle), for_circle_perimetr(digit_circle));} 
	printf("\n");	
...
void triangle_f(char str[]){
	...
	printf("%s\n", str);
	if ((for_treug_area(digit_treug) && for_treug_perimetr(digit_treug)) > 0){printf("\nPERIMETR TRIANGLE: %.2f\nAREA TRIANGLE: %.2f\n", for_treug_perimetr(digit_treug), for_treug_area(digit_treug));}
}
...
void Read(){
	char str[size];
    	...
    	{
		if (str[0] == 'c') circle_f(str);
        	if (str[0] == 't') triangle_f(str);
    	}
}
