...
float for_treug_area(float digit_treug[]){
	float area, p1, a, b, c, x1 = digit_treug[2], x2 = digit_treug[4], x3 = digit_treug[6], y1 = digit_treug[3], y2 = digit_treug[5], y3 = digit_treug[7];
...
float for_treug_perimetr(float digit_treug[]){
	float perimetr, a, b, c, x1 = digit_treug[2], x2 = digit_treug[4], x3 = digit_treug[6], y1 = digit_treug[3], y2 = digit_treug[5], y3 = digit_treug[7];
	a = sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
    	b = sqrt((x3 - x2) * (x3 - x2) + (y3 - y2) * (y3 - y2));
    	c = sqrt((x1 - x3) * (x1 - x3) + (y1 - y3) * (y1 - y3));
	if (a > 0 || b > 0 || c > 0) {
...
void for_all_circle(float digit_treug[], float digit_circle[]){
	...
	if (cross(all)){printf("NOT CROSSING\n");}
	else{printf("CROSSING\n");}
}
